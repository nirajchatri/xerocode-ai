...
import { Copy, Palette, Plus, Settings, Sparkles, Trash2 } from 'lucide-react';
...
const AGENT_PREVIEW_PLACEHOLDER = 'Agent Name';

export function AgentLlmWorkflowNode({ id, data, selected }: NodeProps) {
  const { setNodes, setEdges, getNode } = useReactFlow();
  const studio = useAgentStudioActions();
  const d = (data || {}) as AgentNodeData;
  const bar = dataListBarClass(d.agentBarAccent);
  const listAccent = d.agentBarAccent ?? 'violet';

  const titleLine = String(d.agentDisplayName ?? '').trim() || AGENT_PREVIEW_PLACEHOLDER;
    ...
    <div ref={nodeRef} className="relative" onMouseEnter={onEnter} onMouseLeave={onLeave}>
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            title="Agent settings — side panel"
        ...
        <div className="space-y-1.5 p-2">
          <p
            className={`truncate text-[10px] font-semibold ${
              (d.agentDisplayName || '').trim() ? 'text-slate-700 dark:text-slate-200' : 'text-slate-400 dark:text-slate-500'
            }`}
          >
            {titleLine}
          </p>
          ...
          <div className="rounded border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
            {truncate(String(d.model || ''), 28) || 'Model not set'}
          </div>
        </div>
      </div>
    </div>
