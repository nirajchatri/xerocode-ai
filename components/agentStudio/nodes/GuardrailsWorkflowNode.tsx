...
import { Copy, Palette, Plus, Settings, Shield, Trash2 } from 'lucide-react';

import type { AgentNodeData } from '../agentNodeData';
import type { GuardrailCheckId } from '../guardrailsState';
...
const GUARDRAIL_CHECK_ORDER: GuardrailCheckId[] = [
  'pii',
  'moderation',
  'jailbreak',
  'hallucination',
...
export function GuardrailsWorkflowNode({ id, data, selected }: NodeProps) {
  const { setNodes, setEdges, getNode } = useReactFlow();
  const studio = useAgentStudioActions();
  const d = (data || {}) as AgentNodeData;
  const bar = dataListBarClass(d.guardrailBarAccent ?? 'rose');
  const accent = d.guardrailBarAccent ?? 'rose';

  const normalized = useMemo(() => normalizeGuardrailsState(d.guardrailsState, d.blockPii), [d.guardrailsState, d.blockPii]);
  const displayTitle = normalized.displayName?.trim() || 'Guardrails';
  const subtitle = useMemo(() => String(d.subtitle ?? '').trim(), [d.subtitle]);

  const enabledCount = useMemo(
    () => GUARDRAIL_CHECK_ORDER.reduce((acc, key) => acc + (normalized.checkEnabled[key] ? 1 : 0), 0),
    [normalized.checkEnabled]
  );

  const summaryLine = useMemo(() => {
    if (enabledCount > 0) return `${enabledCount} ${enabledCount === 1 ? 'check' : 'checks'} enabled`;
    const legacy = String(d.rulesText ?? '').trim();
    if (legacy) return truncate(legacy.split(/\r?\n/).find(Boolean) ?? legacy, 40);
    return 'No checks enabled';
  }, [enabledCount, d.rulesText]);
  ...
  const onDuplicate = useCallback(() => {
    const me = getNode(id);
    if (!me) return;
    const newId = `guardrails-${Date.now()}`;
  ...
  return (
    <div ref={nodeRef} className="relative" onMouseEnter={onEnter} onMouseLeave={onLeave}>
      ...
      >
        <div className={guardrailToolbarClass} onPointerDown={(e) => e.stopPropagation()} onMouseEnter={onEnter} onMouseLeave={onLeave}>
          <button
            type="button"
            title="Add Guardrails node"
            aria-label="Add Guardrails node"
            onClick={() => studio?.addGuardrailsNode()}
            className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded border border-rose-500/45 bg-rose-500/10 text-rose-800 transition-colors hover:bg-rose-500/20 dark:border-rose-400/45 dark:text-rose-200 dark:hover:bg-rose-500/15"
          >
          ...
          <button
            type="button"
            title="Guardrails settings — side panel"
              ...
              <div className="absolute left-1/2 top-full z-50 mt-1 flex -translate-x-1/2 gap-1.5 rounded-lg border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-600 dark:bg-slate-900">
                {DATA_LIST_BAR_SWATCHES.map(({ key, header }) => (
                  <button
                    ...
                    title={key}
                    onClick={() => {
                      updateData({ guardrailBarAccent: key });
        ...
        <div className="space-y-1.5 p-2">
          <p
            className={`truncate text-[10px] font-semibold ${
              displayTitle !== 'Guardrails' ? 'text-slate-700 dark:text-slate-200' : 'text-slate-400 dark:text-slate-500'
            }`}
          >
            {displayTitle}
          </p>
          {subtitle ? <p className="text-[10px] text-slate-500 dark:text-slate-400">{subtitle}</p> : null}
          <div className="rounded border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
            {summaryLine}
          </div>
        </div>
      </div>
